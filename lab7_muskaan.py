...
def remove_duplicates_set(s):
    """
    This function removes the duplicates from the string s using a set to track seen characters.
    Utilizing a set improves efficiency in checking for duplicates.

    Parameters:
    - s (string): The string from which duplicated chars are removed.
...
# Your unit tests
assert remove_duplicates_set("apple") == "aple", "Test case 1 failed"
assert remove_duplicates_set("Popsipple") == "Popsile", "Test case 2 failed"
assert remove_duplicates_set("pear") == "pear", "Test case 3 failed"
assert remove_duplicates_set("Apple") == "Aple", "Test case 4 failed"
...
def gem_counting(stones, gems):
    """
    Counts how many stones are also considered as gems. A stone is considered a gem if its type is present in the gems string.

    Parameters:
    - stones (string): A string where each character represents a type of stone collected.
    - gems (string): A string where each character represents a type of gem.

    Returns:
    - int: The number of stones that are also gems.
    """
    gem_types = set(gems)  # Convert gems to a set for efficient look-up.
    count = 0
    for stone in stones:
        if stone in gem_types:
            count += 1
    return count

# Your unit tests
assert gem_counting("abDFMdm", "admMQq") == 4, "Test case 1 failed"
assert gem_counting("abDFMdm", "af") == 1, "Test case 2 failed"
assert gem_counting("awCcM", "cQqW") == 1, "Test case 3 failed"
assert gem_counting("bFfL", "cQqW") == 0, "Test case 4 failed"
...
def students_id(ids):
    """
    Calculates the number of unique students who use the shuttle service by analyzing a list of student IDs.
    Each student ID in the list is considered as one use of the shuttle service.
    Since a student may use the service multiple times, this function counts each student only once.

    Parameters:
    - ids (list of str): A list containing the student IDs of those who have used the shuttle service.

    Returns:
    - int: The number of unique students who have used the shuttle service.
    """
    unique_students = set(ids)  # Convert the list to a set to automatically remove duplicates.
    return len(unique_students)

# Your unit tests
assert students_id(['002', '003', '001', '004', '012']) == 5, "Test case 1 failed"
assert students_id(['002', '003', '001', '012', '003', '001']) == 4, "Test case 2 failed"
...
def students_id_occurrences(ids):
    """
    Counts the occurrences of each unique student ID in a list, indicating how many times each student has used the shuttle service.
    
    Parameters:
    - ids (list of str): A list containing the student IDs of those who have used the shuttle service.
    
    Returns:
    - dict: A dictionary where each key is a student ID and its value is the number of times that student has used the shuttle service.
    """
    id_dict = {}
    for id in ids:
        if id in id_dict:
...
# Your unit tests
assert students_id_occurrences(['002', '003', '001', '004', '012']) == {'002': 1, '003': 1, '001': 1, '004': 1, '012': 1}, "Test case 1 failed"
assert students_id_occurrences(['002', '003', '001', '012', '003', '001']) == {'002': 1, '003': 2, '001': 2, '012': 1}, "Test case 2 failed"
...
def word_frequency(paragraph):
    """
    Counts the frequency of each word in a given paragraph. The function treats words as case-sensitive and does not separate words from punctuation.
    
    Parameters:
    - paragraph (str): A string containing the paragraph to analyze.
    
    Returns:
    - dict: A dictionary where each key is a word from the paragraph and its value is the frequency of that word.
    """
...
# Your unit tests
assert word_frequency("I am alive. I am happy.") == {'I': 2, 'am': 2, 'alive.': 1, 'happy.': 1}, "Test case 1 failed"
assert word_frequency("I do not like water. I like fruits.") == {'I': 2, 'do': 1, 'not': 1, 'like': 2, 'water.': 1, 'fruits.': 1}, "Test case 2 failed"
